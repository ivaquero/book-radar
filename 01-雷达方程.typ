...
其中，$P_t$和$P_r$分别为雷达的发射功率和接收功率，$G_t$和$G_r$分别为天线的发射增益和接收增益，$R$为探测距离，$σ$为雷达反射截面积（radar cross section，RCS），$L$为能量损失，$F$为传播因子，$λ$为信号频率。其中
...
$
  "mix" = sin (a t) ⋅ sin (b t)\
  "mix" = 1 / 2 [cos underbrace((a - b), "beat frequency") t - cos (a + b) t]
$

拍频（beat frequency）又称差频，指两个频率相近但不同的声波的干涉，所得到的干涉信号的频率是原先两个声波的频率之差的绝对值。雷达中，拍频通常由 Doppler 效应产生，是传输频率（transmission frequency，TF）和接收频率（reception frequency，RF）之间的差值。

=== 调频

对于静止的物体，差频为$0$，信号沿水平方向移动，这个时候需要对频率进行调制（modulation）。

=== 线性调制

线性调制是将传输频率增加到另一个值，创建锯齿状调制模式。当接收信号有延迟时，每时每刻会得到不同的频率。由产生的差频，可以计算出量测的位

一个经验法则，Carson 法则指出几乎所有（≈98%）的调频信号的功率处于带宽。
