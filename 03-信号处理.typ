...
对于$k$阶 FIR 滤波器，输出由最后$k + 1$个输入的加权和确定，权重为$w_k$
...
FIR 滤波器和 IIR 滤波器都是线性滤波器，因为滤波器系数仅线性地进入传递函数。虽然这种线性滤波器擅长消除高斯分布的噪声，但它无法完成其他任务，如用于消除极端异常值。数据中的此类峰值可能是由以下原因引起的：由于传感器故障或实验装置中的连接松动。对于此类任务，所谓的“形态滤波器”可能很有用。这些形态滤波器不使用输入/输出的加权平均值，而是使用数据特征，如数据窗口内的元素的最值、中值、众数等。简单来说，为了消除极端异常值，中值滤波器提供比线性滤波器更好的噪声抑制。
...
#definition[
  伪影（artifacts）：滤波器初始化值显示的“初始瞬态”，其输出相对于输入总是存在时间延迟。
]

=== 非因果滤波器的伪影

对于离线分析，使用以居中分析窗口（centeredif analysis window）通常更方便。这消除了滤波后的输出信号$y$相对于输入信号$x$的时间延迟问题。在 DSP 语言中，这称为非因果滤波器，因为未来点包含在$y_i$的评估中，这在数据的实时分析中是不可能的。
...
为了平滑曲线，Savitzky-Golay 滤波器将$q$阶多项式拟合到周围$2m + 1$个数据点，并使用其中心的值作为该点的输出。为了计算输入信号的一阶导数，采用拟合多项式中心的倾角，对于二阶导数，采用曲率等。令人惊讶的是，这种多项式拟合可以通过简单的 FIR 滤波器来实现。Savitzky-Golay 滤波器的优点之一是只需确定滤波器系数$w$一次，因为其不依赖于输入信号。
...
方程中的加速度。 @eqt:vel ∼ @eqt:pos 是相对于空间的加速度。然而，在使用惯性传感器进行测量时，加速度是相对于传感器获得的。
...
现代雷达对接收信号都进行了一些形式的采样，而对信号序列（输入序列）$x(n)$的截短不可避免，通常使用乘积来实现。为了减少频谱能量泄漏，可采用不同的截取函数（权重函数）$w(n)$对信号截断，即加窗序列，简称为窗（window）。
