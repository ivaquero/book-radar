...
#figure(
  image("images/filter-ma.png", width: 40%),
  caption: "移动平均滤波器",
  supplement: [图],
)
...
移动平均滤波器快速且简单，但其输出不是很准确。如，它系统地低估了信号峰值附近的真实信号值。这个问题可以通过具有平滑和微分能力的 FIR 滤波器，Savitzky-Golay 滤波器@savitzkySmoothingDifferentiationData1964 来消除。
