...
彩色图像只不过是三个灰度图像的堆叠：一个代表“红色”通道，一个代表“绿色”通道，一个代表“蓝色”通道。如果三种颜色按此顺序堆叠，则该图像称为 RGB 图像。
...
线性滤波器是可交换的。换句话说，这些过滤器的应用顺序并不重要。然而，对数据的形态操作是非线性操作，最终结果取决于顺序。

= 事件和特征查找

通常需要在数据流中查找特定位置。如，人们可能想知道信号何时超过给定阈值。或者，在分析运动信号时，人们可能想找出运动开始的点和结束的点。如果数据是时间序列，这些位置通常称为事件（event）。
...
#figure(
  image("images/cross-corr.png", width: 40%),
  caption: "互相关",
  supplement: [图],
)
