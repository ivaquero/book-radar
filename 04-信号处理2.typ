...
$
  X(f) = ∫_(-∞)^∞ x(t) e^(-j 2π f t) dif t
$

其中，$f$代表频率。逆变换由下式给出

$
  x(t) = ∫_(-∞)^∞ X(k) e^(j 2π f t) dif f
$
...
由于这些方程包含有限数量的离散数据点@eqt:four 和有限数量的离散波@eqt:four_inv，因此这种变换被称为离散 Fourier 变换（Discrete Fourier Transform，DFT）。

由@eqt:four_inv，且 DFT 中的指数在频率上是周期性的，则有
...
高于 Nyquist 频率的频率可以解释为负频率。由于对实值信号，负频率分量是相应正频率的复共轭（@eqt:real），因此可以忽略这些分量，因为它们不包含任何附加信息。这称为单边频谱（single-sided spectrum），如 @sound。
...
由于卷积与互相关密切相关，因此与@eqt:conv 有对应关系：
...
#tip[
  在@eqt:corr 中哪个信号被共轭取决于互相关的定义 - 并且使用不同的定义。
]
